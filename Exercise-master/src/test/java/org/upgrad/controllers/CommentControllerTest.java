...
import org.upgrad.controller.CommentController;
import org.upgrad.model.Comment;
...
public class CommentControllerTest {
    ...
    public void addCommentWithoutLogin() throws Exception {
        ...
        mvc.perform ( post ( url ).session ( session ).param ( "comment", comment ).param ( "answerId", answerId ) ).andExpect ( status ().is4xxClientError () ).andExpect ( content ().string ( Matchers.containsString ( "Please Login first to access this endpoint!" ) ) );
    }
    ...
    public void addCommentWithLogin() throws Exception {
        ...
        String answerId = "1";
        String url = "/api/comment";
        mvc.perform ( post ( url ).session ( session ).param ( "comment", comment ).param ( "answerId", answerId ) ).andExpect ( status ().is2xxSuccessful () ).andExpect ( content ().string ( Matchers.containsString ( " answerId " + answerId + " commented successfully." ) ) );
    }
    ...
    public void editCommentWithoutLogin() throws Exception {
        ...
        mvc.perform ( put ( url ).session ( session ).param ( "commentId", commentId ).param ( "comment", comment ) ).andExpect ( status ().is4xxClientError () ).andExpect ( content ().string ( Matchers.containsString ( "Please Login first to access this endpoint!" ) ) );
    }
    ...
    public void editCommentWithoutAuthorization() throws Exception {
        ...
        String url = "/api/comment/1";
        mvc.perform ( put ( url ).session ( session ).param ( "comment", com ).param ( "commentId", commentId ) ).andExpect ( status ().is4xxClientError () ).andExpect ( content ().string ( Matchers.containsString ( "You do not have rights to edit this comment." ) ) );
    }
    ...
    public void editCommentWithAuthorization() throws Exception {
        ...
        Mockito.when ( commentService.findUserIdfromComment ( 1 ) ).thenReturn ( 2 );
        String url = "/api/comment/1";
        mvc.perform ( put ( url ).session ( session ).param ( "comment", comment ).param ( "commentId", commentId ) ).andExpect ( status ().is2xxSuccessful () ).andExpect ( content ().string ( Matchers.containsString ( "Comment with commentId " + commentId + " edited successfully." ) ) );
    }
